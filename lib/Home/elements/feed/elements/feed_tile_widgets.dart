// ignore_for_file: use_build_context_synchronously, non_constant_identifier_names, constant_identifier_names
import 'package:jayben/Home/elements/feed/elements/components/block_confirmation_dialogue.dart';
import 'package:jayben/Home/elements/messages/elements/send_reply_message_dialogue.dart';
import 'package:jayben/Home/elements/feed/elements/components/report_dialogue.dart';
...
Widget feedTileBody(BuildContext context, Map post_map) {
  return SizedBox(
    width: width(context),
    child: Stack(
      children: [
...
Widget deletePostButton(BuildContext context, Map post_map) {
  ...
  return Positioned(
    ...
    child: GestureDetector(
      onTap: () async {
        if (isPostOwner) {
          showSnackBar(context, "Post is being deleted...");

          // await context
          //     .read<FeedProviderFunctions>()
          //     .deletePost(post_map['post_id']);

          return;
        }

        showSnackBar(context, "Can't delete posts that aren't yours");
      },
      child: !isPostOwner
          ? const SizedBox()
          : SizedBox(
...
Widget menuCard(BuildContext context, Map post_map) {
  bool isPostOwner = post_map["user_id"] == box("user_id");
                  ...
                  children: [
                    GestureDetector(
                      onTap: () async {
                        ...
                        showSnackBar(
                            context, "Can't delete posts that aren't yours");
    ...
    child: Row(
      ...
      children: [
        SizedBox(
          ...
          child: post_transaction_type == "Transfer" &&
        ...
        post_map["post_owner_user_id"] == box("user_id")
            ...
            : DropdownButtonHideUnderline(
                child: DropdownButton2(
                  ...
                  onChanged: (value) =>
                      MenuItems.onChanged(context, value!, post_map),
...
Widget postCaption(BuildContext context, Map post_map) {
  return post_map["post_caption"] == ""
      ...
      : Container(
          width: width(context),
          alignment: Alignment.centerLeft,
          padding: const EdgeInsets.only(bottom: 15.0, top: 5, right: 20),
          child: Text(
          ...
          post_map["post_owner_user_id"] == box("user_id")
              ...
              : GestureDetector(
                  onTap: () {
                    showDialogue(
                      context,
                      SendReplyMessageDialogue(
                        other_person_user_id: post_map["post_owner_user_id"],
                      ),
                      is_dismissble: false,
                    );
...
abstract class MenuItems {
  ...
  static const block_person = MenuItem(text: 'Block');
  static const report_post = MenuItem(text: 'Report');
  ...
  static void onChanged(BuildContext context, MenuItem item, Map post_map) {
    switch (item) {
      case MenuItems.block_person:
        showDialogue(context,
            BlockConfirmDialogue(user_map: post_map["post_owner_details"]));
        break;
      case MenuItems.report_post:
        showDialogue(context, ReportPostDialog(post_map: post_map));
        break;
    }
  }
}
